...
export class PaginationRenderer {
	...
	createPaginationElement() {
		...
		this.paginationElement.className =
			"pagination-container centered-element is-hidden";
	...
	updatePaginationContentOptimized(state) {
		...
		// 渲染首页按钮（如果需要）
		if (
			this.controller.config.showFirstLast &&
			this.shouldShowFirstButton(state)
		) {
			fragment.appendChild(
				this.getOrCreatePageButton(1, state.currentPage === 1),
			);
			globalPerformanceMonitor.incrementDOMOperations();
		...
		state.visiblePages.forEach((pageNum) => {
			...
			if (this.controller.config.showFirstLast) {
				...
				if (pageNum === state.totalPages && this.shouldShowLastButton(state))
					return;
			}

			fragment.appendChild(
				this.getOrCreatePageButton(pageNum, pageNum === state.currentPage),
			);
			globalPerformanceMonitor.incrementDOMOperations();
		});
		...
		if (
			this.controller.config.showFirstLast &&
			this.shouldShowLastButton(state)
		) {
		...
		if (this.controller.config.showFirstLast) {
			...
			if (this.shouldShowLastButton(state)) {
				...
				const lastVisiblePage =
					state.visiblePages[state.visiblePages.length - 1];
	...
	shouldShowFirstButton(state) {
		return (
			state.totalPages > this.controller.config.maxVisiblePages &&
	...
	bindEventsOptimized() {
		if (!this.paginationElement) {
			console.error(
				"[PaginationRenderer] Cannot bind events: element not found",
			);
			return;
		}
		...
		this.paginationElement.addEventListener("click", clickHandler, {
			passive: false,
		});
		...
		this.paginationElement.addEventListener("keydown", keydownHandler, {
			passive: false,
		});
		...
		try {
			if (button.classList.contains("pagination-prev")) {
				...
				if (!Number.isNaN(pageNum)) {
					...
					console.error(
						"[PaginationRenderer] Invalid page number:",
						button.dataset.page,
					);
				}
			}
			...
			console.error(
				"[PaginationRenderer] Button click handling failed:",
				error,
			);
		} finally {
			// 短暂延迟后移除处理状态
	...
	focusPreviousButton(currentButton) {
		const buttons = Array.from(
			this.paginationElement.querySelectorAll(
	 ...
	 */
	focusFirstButton() {
		const firstButton = this.paginationElement.querySelector(
			".pagination-button:not([disabled])",
		);
		if (firstButton) {
			firstButton.focus();
	 ...
	 */
	focusLastButton() {
		const buttons = this.paginationElement.querySelectorAll(
			".pagination-button:not([disabled])",
		);
		const lastButton = buttons[buttons.length - 1];
		if (lastButton) {
	...
	applyResponsiveStyles() {
		if (!this.paginationElement) {
			console.error(
				"[PaginationRenderer] Cannot apply responsive styles: element not found",
			);
			return;
		}
	...
	checkMobileScrollable() {
		...
		try {
			...
			console.error(
				"[PaginationRenderer] Mobile scroll detection failed:",
				error,
			);
		}
	}

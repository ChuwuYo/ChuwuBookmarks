...
export const ErrorHandler = {
    ...
    validatePage(page, totalPages) {
        ...
        if (totalPages > 0 && page > totalPages) {
            throw new PaginationError(`页码不能大于总页数${totalPages}`, ERROR_CODES.INVALID_PAGE);
        }
    },
    ...
    validatePageSize(pageSize) {
        if (typeof pageSize !== 'number' || !Number.isInteger(pageSize)) {
            throw new PaginationError('每页项目数必须是整数', ERROR_CODES.INVALID_PAGE_SIZE);
        }

        if (pageSize < 1) {
            throw new PaginationError('每页项目数不能小于1', ERROR_CODES.INVALID_PAGE_SIZE);
        }

        if (pageSize > 1000) {
            throw new PaginationError('每页项目数不能大于1000', ERROR_CODES.INVALID_PAGE_SIZE);
        }
    },
    ...
    validateTotalItems(totalItems) {
        if (typeof totalItems !== 'number' || !Number.isInteger(totalItems)) {
            throw new PaginationError('总项目数必须是整数', ERROR_CODES.INVALID_TOTAL_ITEMS);
        }

        if (totalItems < 0) {
            throw new PaginationError('总项目数不能为负数', ERROR_CODES.INVALID_TOTAL_ITEMS);
        }
    },
    ...
    validateDataArray(data) {
        if (!Array.isArray(data)) {
            throw new PaginationError('数据必须是数组类型', ERROR_CODES.DATA_TYPE_ERROR);
        }
    },
    ...
    validateDOMElement(element, elementName = '元素') {
        if (!element || !(element instanceof HTMLElement)) {
            throw new PaginationError(`${elementName}必须是有效的HTML元素`, ERROR_CODES.DOM_NOT_FOUND);
        }
    },
    ...
    validateController(controller) {
        if (!controller || typeof controller !== 'object') {
            throw new PaginationError('分页控制器不能为空', ERROR_CODES.CONTROLLER_NOT_INITIALIZED);
        }

        if (typeof controller.calculatePagination !== 'function') {
            throw new PaginationError('控制器缺少必要的方法', ERROR_CODES.CONTROLLER_NOT_INITIALIZED);
        }
    },
    ...
    batchValidate(validations) {
        ...
        if (errors.length > 0) {
            throw new PaginationError(`参数验证失败: ${errors.join(', ')}`, ERROR_CODES.DATA_TYPE_ERROR);
        }
    },
        ...
        return (...args) => {
            ...
            try {
                ...
                if (duration > maxDuration && window.globalPerformanceMonitor) {
                    window.globalPerformanceMonitor.recordSlowOperation(operationName, duration);
                }

...
class SearchResultsManager {
	...
	handlePageChange(newPage, allResults, container, renderMainContent) {
		...
		const totalPages = Math.ceil(
			allResults.length / this.paginationController.config.itemsPerPage,
		);
		...
		// 重新计算分页状态
		const paginationState = this.paginationController.calculatePagination(
			allResults.length,
			newPage,
		);

		// 渲染新页面的结果
		this.renderCurrentPageResults(
			allResults,
			paginationState,
			container,
			renderMainContent,
	...
	renderCurrentPageResults(
		...
		requestAnimationFrame(() => {
			...
			const currentPageData =
				this.paginationController.getCurrentPageData(allResults);
			...
			if (content) {
				...
				setTimeout(() => {
					content.classList.remove(
						"search-results-fade",
						"search-results-fade-full",
					);

					// 优化焦点设置
					const firstResult = content.querySelector(
						'.search-result-item[tabindex="0"]',
					);
					if (firstResult) {
						firstResult.focus();
					}
	...
	adjustPaginationForLayout(layoutData) {
		...
		if (this.paginationRenderer?.paginationElement) {
			...
			// 根据设备类型调整样式类
			paginationElement.classList.toggle(
				"mobile-layout",
				deviceType === "mobile",
			);
			paginationElement.classList.toggle(
	...
	getPaginationState(targetPage = 1) {
		...
		return this.paginationController.calculatePagination(
	...
	createPaginationControls(content, paginationState) {
		if (paginationState.totalPages > 1) {
			const paginationContainer = PaginationRenderUtils.createContainer(
				content,
				"pagination-wrapper",
			);
			...
			const paginationElement = paginationContainer.querySelector(
				".pagination-container",
			);
			if (paginationElement) {
				PaginationRenderUtils.initializeResponsiveSupport(
					paginationElement,
					this.paginationController,
				);
			}
...
const renderSearchResults = (results, renderMainContent) => {
	...
	// 使用搜索结果管理器初始化
	const sortedResults = searchResultsManager.initialize(
		results,
		container,
		renderMainContent,
	);
	...
	if (paginationState) {
		// 渲染当前页结果
		searchResultsManager.renderCurrentPageResults(
			sortedResults,
			paginationState,
			container,
			renderMainContent,

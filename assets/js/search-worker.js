...
self.addEventListener("message", (e) => {
	...
	switch (action) {
		case "search": {
			...
			const indexKey =
				data.indexHash || (index ? index.length : bookmarks.length);
...
function searchBookmarks(keyword, data, options = {}) {
	...
	const searchInTitle =
		searchFields.includes("title") || searchFields.includes("all");
	const searchInUrl =
		searchFields.includes("url") || searchFields.includes("all");

	// 检测是否为扁平索引（预处理索引）
	const isFlatIndex =
		Array.isArray(data) && data.length > 0 && data[0].__lcTitle !== undefined;

	// 公共匹配逻辑
	const itemMatches = (item) => {
		let isMatch = false;
		const lcTitle = isFlatIndex
			? item.__lcTitle || item.title.toLowerCase()
			: item.title?.toLowerCase();
		const lcUrl = isFlatIndex
			? item.__lcUrl || item.url.toLowerCase()
